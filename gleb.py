...
# Мы можем видеть, что набор данных содержит 9358 экземпляров и 15 столбцов.
# Есть некоторые недостающие значения, обозначаемые -200.
# Нам нужно будет обработать эти недостающие значения на этапе предварительной обработки данных.

#2

# Визуализируем распределение концентраций CO
sns.histplot(df['CO(GT)'], kde=True)
plt.show()

# Визуализируем распределение концентраций С6H6(бензол)
